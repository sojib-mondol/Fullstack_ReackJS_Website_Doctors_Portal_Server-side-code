...
function verifyJWT(req, res, next){
  ...
  if(!authHeader){
    return res.send(401).send('unauthorize access');
  }
...
async function run() {
  try{
    ...
    // Use Aggregate to qury multiple collection and then merge data
    ...
    app.get('/bookings', verifyJWT, async(req, res) => {
      ...
      const bookikngs = await bookingsCollection.find(query).toArray();
      res.send(bookikngs); 
    })
    ...
    app.post('/bookings', async(req, res) => {
      ...
      if(alredyBooked.length){
        const message = `Your alredy have a booking on ${booking.appointmentDate}`;
  ...
  //temporary to update price field on apponitment options 
